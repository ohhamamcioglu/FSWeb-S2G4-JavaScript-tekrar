...
console.log("3'e tam bölünenler (adet):", ucetambolunenler.length);
...
console.log("500'den küçükler (adet):", besyuzdenkucuksayilar.length);

// 3e çözümü

siralisayilar = [...besyuzdenkucuksayilar].sort((a, b) => a - b);
console.log("Sıralı (ilk 5):", siralisayilar.slice(0, 5));

// 3f çözümü

const sayim = {}; // {deger: adet}
sayilar.forEach((n) => {
  const key = String(n); // floatlar da dahil
  sayim[key] = (sayim[key] || 0) + 1;
});
tekraredensayilar = [];
for (const [deger, adet] of Object.entries(sayim)) {
  if (adet > 1) {
    tekraredensayilar.push(`${deger} sayısı ${adet} kere tekrar edilmiştir`);
  }
}
console.log("Tekrar edenler (örnek 5):", tekraredensayilar.slice(0, 5));
